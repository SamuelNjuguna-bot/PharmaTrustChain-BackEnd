...
exports.Payment = exports.Stock = void 0;
...
Stock.init(
  {
    ...
    batchId: { type: sequelize_1.DataTypes.INTEGER, allowNull: false },
    ownerWallet: { type: sequelize_1.DataTypes.STRING, allowNull: false },
    role: { type: sequelize_1.DataTypes.INTEGER, allowNull: false },
...
Payment.init(
  {
    ...
    payerWallet: { type: sequelize_1.DataTypes.STRING, allowNull: false },
    payeeWallet: { type: sequelize_1.DataTypes.STRING, allowNull: false },
    amount: { type: sequelize_1.DataTypes.FLOAT, allowNull: false },
    phone: { type: sequelize_1.DataTypes.STRING, allowNull: false },
    method: { type: sequelize_1.DataTypes.STRING, defaultValue: "MPESA" },
    checkoutRequestId: { type: sequelize_1.DataTypes.STRING, allowNull: true },
    mpesaReceipt: { type: sequelize_1.DataTypes.STRING, allowNull: true },
    status: {
...
// -------------------- BLOCKCHAIN SETUP --------------------
const contractPath = path_1.default.join(
  __dirname,
  "../contracts/PharmaTrustChain.json",
);
...
const contractJson = JSON.parse(
  fs_1.default.readFileSync(contractPath, "utf8"),
);
...
app.post("/pinata/upload", async (req, res) => {
  try {
    ...
    const ipfsHash = raw.IpfsHash || raw.ipfsHash || null;
    if (!ipfsHash) {
...
app.get("/manufacturer/batches", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error fetching manufacturer batches" });
  }
});
...
app.get("/stock/distributor/:wallet", async (req, res) => {
...
app.get("/orders/manufacturer/:wallet", async (req, res) => {
  try {
    ...
    const orders = await Order.findAll({
      where: sequelize_1.Sequelize.where(
        sequelize_1.Sequelize.fn("lower", sequelize_1.Sequelize.col("seller")),
        wallet,
      ),
    });
...
//       });
//     }
//     // 2️⃣ Call blockchain (ADMIN / OPERATOR)
...
//       });
//     }
//      const sellerr = seller?.toLowerCase();
//      console.log(sellerr)
//     // 3️⃣ Fetch manufacturer stock
...
//       lock: transaction.LOCK.UPDATE,
//     });
//     if (!manufacturerStock || manufacturerStock.quantity < orderQuantity) {
//       await transaction.rollback();
//       return res.status(400).json({
//         success: false,
//         message: "Insufficient manufacturer stock",
//       });
//     }
//     // 4️⃣ Decrease manufacturer stock
//     await manufacturerStock.decrement(
//       { quantity: orderQuantity },
//       { transaction }
//     );
//     // 5️⃣ Increase / create distributor stock
...
//     });
//     if (distributorStock) {
//       await distributorStock.increment(
//         { quantity: orderQuantity },
//         { transaction }
//       );
//     } else {
...
//         from: seller,
//         to: buyer,
//         action: "Ownership transferred & stock reconciled",
...
//     return res.json({
//       success: true,
//       message: "Payment confirmed, ownership transferred, stock updated",
//       txHash: receipt.transactionHash,
//     });
//   } catch (err) {
//     await transaction.rollback();
...
async function initiateSTKPush(orderId, phone, amount) {
  ...
  const payload = {
    ...
    PhoneNumber: phone,
    CallBackURL: "https://your-backend.com/orders/mpesa/callback",
    AccountReference: `Order-${orderId}`,
    TransactionDesc: `PharmaTrustChain Order ${orderId}`,
  };
  const res = await axios_1.default.post(stkUrl, payload, {
...
//     const items = callback.Body.stkCallback.CallbackMetadata?.Item as { Name: string; Value: any }[] | undefined;
...
//       await payment.save();
//       // ✅ Trigger your existing fulfillment logic (same as old /orders/pay)
...
//     // ✅ Existing logic: transferOwnership on blockchain
...
// stock Update route
app.get("/stock/manufacturer/:wallet", async (req, res) => {
  try {
    const wallet = (req.params.wallet || "").toLowerCase();
    const stock = await Stock.findAll({
...
app.get("/batches/distributor/:wallet", async (req, res) => {
  try {
    ...
    const ownedBatches = formatted.filter(
      (b) => b.currentOwner.toLowerCase() === wallet,
    );
    // 🔹 4. Merge and return unique list
    const combined = [
      ...availableBatches,
      ...ownedBatches.filter(
        (ob) => !availableBatches.some((ab) => ab.id === ob.id),
      ),
    ];
...
app.post("/pharmacy-orders/:id/confirm-payment", async (req, res) => {
  try {
    ...
    if (receipt.status === 1) {
      ...
      if (!distributorStock || distributorStock.quantity < orderQuantity) {
        return res.status(400).json({
          success: false,
          message: "Insufficient distributor stock",
        });
      }
      // 2️⃣ Decrease distributor stock
      await distributorStock.decrement({ quantity: orderQuantity });
      // 3️⃣ Fetch pharmacy stock
      ...
      // 4️⃣ Increase or create pharmacy stock
      if (pharmacyStock) {
        await pharmacyStock.increment({ quantity: orderQuantity });
      } else {
        await Stock.create({
          batchId: order.batchId,
          ownerWallet: order.pharmacy.toLowerCase(),
